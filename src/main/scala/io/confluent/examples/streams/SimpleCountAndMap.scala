...
/**
  ...
  *
  * {{{
  * $ java -cp ~/kafka-streams-examples-5.4.0-standalone.jar io.confluent.examples.streams.SimpleCountAndMap
  * }}}
  *
  ...
  **/
object SimpleCountAndMap extends App {
  ...
  val config: Properties = {
    ...
    p.put(StreamsConfig.COMMIT_INTERVAL_MS_CONFIG, 10L)
    p.put(StreamsConfig.POLL_MS_CONFIG, 10L)
    p
  }

  val builder = new StreamsBuilder()

  val pageIndex = Map("page1" -> "1", "page2"-> "2", "page3" -> "3", "page4" -> "4")

 // val textLines: KStream[String, String] = builder.stream[String, String]("streams-plaintext-input")
  val textLines: KStream[String, String] = builder.stream[Array[Byte], String]("streams-plaintext-input")
    .map((_, value) => (value , pageIndex.apply(value)))
